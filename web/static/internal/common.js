...
function isFreeFirstEnabled() {
  return !!getLocalStorageItem("channelOrderFreeFirst", true);
}

function updateChannelOrderUI() {
  const btn = document.getElementById("channel-order-toggle");
  const label = document.getElementById("channel-order-label");
  const enabled = isFreeFirstEnabled();

  if (btn) {
    btn.checked = enabled;
  }
  if (label) {
    label.textContent = enabled ? "Free first" : "Legacy order";
  }
}

function syncChannelOrderModeToUrl() {
  if (!document.querySelector("a.card[data-channel-id]")) {
    return;
  }

  const enabled = isFreeFirstEnabled();
  const currentSort = getCurrentUrlParams().get("sort") || "";
  const desiredSort = enabled ? "" : "legacy";

  if (currentSort === desiredSort) {
    return;
  }

  updateUrlParameter("sort", desiredSort);
  window.location.reload();
}

function toggleChannelOrderMode() {
  const btn = document.getElementById("channel-order-toggle");
  const next = btn && typeof btn.checked !== "undefined" ? !!btn.checked : !isFreeFirstEnabled();

  setLocalStorageItem("channelOrderFreeFirst", next);
  updateChannelOrderUI();

  if (document.querySelector("a.card[data-channel-id]")) {
    updateUrlParameter("sort", next ? "" : "legacy");
    window.location.reload();
  }
}
